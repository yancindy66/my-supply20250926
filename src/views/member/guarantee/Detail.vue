  ...
  <div class="detail-container" v-if="detail">
    ...
    <div class="group">
      ...
      <div class="grid">
        ...
        <div><label>许可证编号</label><span>{{ detail.license_no || '—' }}</span></div>
        <div><label>机构类型</label><span>{{ detail.type || '—' }}</span></div>
        <div><label>注册资本(万元)</label><span>{{ detail.registeredCapital || '—' }}</span></div>
        <div><label>放大倍数</label><span>{{ detail.leverage || '—' }}</span></div>
        <div><label>担保责任余额</label><span>{{ detail.guaranteeOutstanding || '—' }}</span></div>
      </div>
    </div>
    <div class="group">
    ...
    <div class="group">
      ...
      <div class="grid">
        <div class="col-2"><label>主要担保业务类型</label><span class="ellipsis" :title="detail.mainGuaranteeTypes">{{ detail.mainGuaranteeTypes || '—' }}</span></div>
