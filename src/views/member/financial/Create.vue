<template>
  <div class="create-owner-container">
    ...
    <form @submit.prevent="onSubmit">
      ...
      <div>
        <label>机构名称：</label>
        <input v-model="form.name" required placeholder="例如：XX银行XX分行" />
      </div>
      <div>
      ...
      <div>
        <label>金融机构许可证编号：</label>
        <input v-model="form.license_number" required />
      </div>
      <div>
        <label>机构类型：</label>
      ...
      <div>
        <label>业务联系人：</label>
        <input v-model="form.contact_person" required />
      </div>
      <div>
        <label>联系人部门及职务：</label>
        <input v-model="form.contact_title" required />
      </div>
      <div>
        <label>联系人手机：</label>
      ...
      <div>
        <label>联系人邮箱：</label>
        <input v-model="form.contact_email" required />
      </div>

      <h3>3. 业务信息</h3>
      <div>
        <label>核心业务类型：</label>
        <input v-model="form.core_business_types" required placeholder="逗号分隔：应收账款保理, 存货质押融资..." />
      </div>
      <div>
        <label>合作起始日期：</label>
      ...
      <div>
        <label>授信额度（平台参考）：</label>
        <input v-model="form.credit_limit" placeholder="单位可自定义，如：5000万" />
      </div>
      ...
      <div>
        <label>数据接口信息（加密存储）：</label>
        <textarea v-model="form.api_info" rows="3" placeholder="如需对接，记录API地址、密钥等"></textarea>
      </div>
...
function onMultiFile(e: Event){
  ...
  arr.forEach(f=>{
    ...
    reader.onload = () => { formFiles.value.push({ name: f.name, dataUrl: String(reader.result||'') }); };
...
function validateForm(): boolean {
  const must = ['name','creditCode','license_number','type','regAddress','bizAddress','servicePhone','contact_person','contact_title','contact_phone','contact_email','core_business_types','coop_start_date','coop_status','admission_status','admin_account'];
...
async function onSubmit(){
  ...
  try{
    const payload = { ...form.value, qualification_files: formFiles.value };
    await financialApi.create(payload);
    message.value='提交成功';
    setTimeout(()=> router.push('/operation/member/financial-institutions'), 300);
