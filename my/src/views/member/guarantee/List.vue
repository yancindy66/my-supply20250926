      ...
      <table class="owner-table">
        ...
        <tbody>
          <tr v-for="(row, idx) in rows" :key="row.id">
            ...
            <td>{{ row.type }}</td>
            <td class="num">{{ row.registeredCapital }}</td>
            <td>{{ row.leverage }}</td>
            <td class="num">{{ row.guaranteeOutstanding }}</td>
            ...
            <td class="text-ellipsis" :title="row.mainGuaranteeTypes">{{ row.mainGuaranteeTypes }}</td>
            ...
            <td class="op-cell sticky-right">
              <button class="op-btn info sm" title="查看" @click="goDetail(row.id)">
...
const templateUrl = '/templates/guarantee-agencies-template.csv';
const importRef = ref<HTMLInputElement|null>(null);
const moreOpen = ref(false);
const totalPages = computed(() => Math.max(1, Math.ceil(total.value / pageSize.value)));
// 密度设置（与其它列表一致）
const density = ref<'compact'|'comfortable'>((localStorage.getItem('ga_density') as any) || (window.innerWidth >= 1920 ? 'comfortable' : 'compact'));
const densityLabel = computed(()=> density.value==='compact' ? '紧凑' : '宽松');
function toggleDensity(){ density.value = density.value==='compact' ? 'comfortable' : 'compact'; localStorage.setItem('ga_density', density.value); }
...
async function load(){
  loading.value = true;
  try{
    const res:any = await guaranteeApi.list({ keyword: keyword.value, page: page.value, pageSize: pageSize.value, sortBy: sortBy.value, sortOrder: sortOrder.value });
...
function goCreate(){ router.push('/operation/member/guarantee-agencies/create'); }
function goEdit(id:number){ router.push(`/operation/member/guarantee-agencies/edit/${id}`); }
function goDetail(id:number){ router.push(`/operation/member/guarantee-agencies/detail/${id}`); }
function confirmDelete(id:number){ confirmId.value=id; showConfirm.value=true; }
async function deleteOne(id:number){ await guaranteeApi.delete(id); showConfirm.value=false; confirmId.value=null; await load(); }

function openBatchConfirm(){ if(selectedIds.value.length>0) showBatchConfirm.value=true; }
async function confirmBatchDelete(){ const ids=[...selectedIds.value]; for(const id of ids){ try{ await guaranteeApi.delete(id);} catch{} } selectedIds.value=[]; showBatchConfirm.value=false; await load(); }
...
async function exportCsv(){
  ...
  const res = await fetch(`/api/member/guarantee/list?${params.toString()}`);
  if(!res.ok) return alert('导出失败');
  const data = await res.json();
  const rows:any[] = data?.data?.list || [];
  const header = ['机构名称','统一社会信用代码','许可证编号','机构类型','注册资本(万元)','放大倍数','担保责任余额','注册地址','经营地址','客服电话','业务联系人','联系人部门及职务','联系人手机','联系人邮箱','合作金融机构数','主要担保业务类型','合作起始日期','合作状态','风险评级','准入审核状态','资质文件数','管理员账号','备注'];
  ...
  const link=document.createElement('a'); link.href=url; link.download=`guarantee-agencies-${Date.now()}.csv`; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
}
...
async function exportCsvServer(){
  ...
  const url = `/api/member/guarantee/export?${usp.toString()}`;
