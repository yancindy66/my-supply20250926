  ...
  <div class="owner-list-container" :class="['density-'+density]">
      ...
      <table class="owner-table">
        <thead>
          <tr>
            ...
            <th class="sortable name-col" @click="toggleSort('name')">仓库名称 {{ sortIndicator('name') }}</th>
            <th>统一社会信用代码</th>
            <th>仓库类型</th>
            <th>仓库地址</th>
            <th>仓库面积(㎡)</th>
        ...
        <tbody>
          <tr v-for="(row, idx) in rows" :key="row.id">
            ...
            <td class="text-ellipsis" :title="row.address">{{ row.address }}</td>
            <td class="num">{{ row.area }}</td>
            <td class="num">{{ row.capacity }}</td>
            <td class="num">{{ row.maxLoad }}</td>
            <td class="text-ellipsis" :title="row.securityDesc">{{ row.securityDesc }}</td>
            <td class="text-ellipsis" :title="row.securityVendor">{{ row.securityVendor }}</td>
            <td>{{ row.iotAccess }}</td>
            <td>{{ Array.isArray(row.partner_financial_ids)? row.partner_financial_ids.length : 0 }}</td>
            <td>{{ row.coop_start_date }}</td>
            <td><span class="tag" :class="statusClass(row.coop_status)">{{ row.coop_status }}</span></td>
            <td>{{ row.headName }}</td>
            <td>{{ row.headPhone }}</td>
            <td class="text-ellipsis" :title="row.headEmail">{{ row.headEmail }}</td>
    ...
    <div v-if="showConfirm" class="confirm-overlay" @click.self="showConfirm=false">
      <div class="confirm-dialog">
        <div class="confirm-title">删除仓库</div>
    ...
    <div v-if="showBatchConfirm" class="confirm-overlay" @click.self="showBatchConfirm=false">
      <div class="confirm-dialog">
        <div class="confirm-title">批量删除仓库</div>
...
const templateUrl = '/templates/supervised-warehouses-template.csv';
const importRef = ref<HTMLInputElement|null>(null);
const moreOpen = ref(false);
const totalPages = computed(() => Math.max(1, Math.ceil(total.value / pageSize.value)));
// 密度设置（与其它列表一致）
const density = ref<'compact'|'comfortable'>((localStorage.getItem('sw_density') as any) || (window.innerWidth >= 1920 ? 'comfortable' : 'compact'));
const densityLabel = computed(()=> density.value==='compact' ? '紧凑' : '宽松');
function toggleDensity(){ density.value = density.value==='compact' ? 'comfortable' : 'compact'; localStorage.setItem('sw_density', density.value); }
...
async function load(){
  loading.value = true;
  try{
    const res:any = await warehouseApi.list({ keyword: keyword.value, page: page.value, pageSize: pageSize.value, sortBy: sortBy.value, sortOrder: sortOrder.value });
...
function goCreate(){ router.push('/operation/member/supervised-warehouses/create'); }
function goEdit(id:number){ router.push(`/operation/member/supervised-warehouses/edit/${id}`); }
function goDetail(id:number){ router.push(`/operation/member/supervised-warehouses/detail/${id}`); }
function confirmDelete(id:number){ confirmId.value=id; showConfirm.value=true; }
async function deleteOne(id:number){ await warehouseApi.delete(id); showConfirm.value=false; confirmId.value=null; await load(); }

function openBatchConfirm(){ if(selectedIds.value.length>0) showBatchConfirm.value=true; }
async function confirmBatchDelete(){ const ids=[...selectedIds.value]; for(const id of ids){ try{ await warehouseApi.delete(id);} catch{} } selectedIds.value=[]; showBatchConfirm.value=false; await load(); }
...
async function exportCsv(){
  ...
  const res = await fetch(`/api/member/warehouse/list?${params.toString()}`);
  if(!res.ok) return alert('导出失败');
  const data = await res.json();
  const rows:any[] = data?.data?.list || [];
  const header = ['仓库名称','统一社会信用代码','仓库类型','仓库地址','仓库面积(㎡)','可用容积(m³)','最大承重(吨/㎡)','安保措施描述','主要监控服务商','物联网接入','合作金融机构数','合作起始日期','合作状态','负责人姓名','负责人手机','负责人邮箱','风险评级','准入审核状态','资质文件数','管理员账号','备注'];
  ...
  const lines = [header.join(',')].concat(rows.map(r=>[
    ...
    r.securityVendor,
    r.iotAccess,
    Array.isArray(r.partner_financial_ids)? r.partner_financial_ids.length : 0,
    r.coop_start_date,
    r.coop_status,
    r.headName,
    r.headPhone,
  ...
  const link=document.createElement('a'); link.href=url; link.download=`supervised-warehouses-${Date.now()}.csv`; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
}
...
async function exportCsvServer(){
  ...
  const url = `/api/member/warehouse/export?${usp.toString()}`;
...
.more-panel { position: sticky; top: 0; z-index: 12; margin-bottom: 8px; padding: 8px 10px; border-radius: 12px; background: linear-gradient(180deg, rgba(255,255,255,.52), rgba(236,244,255,.48)); backdrop-filter: blur(10px) saturate(1.05); box-shadow: 0 6px 18px rgba(15,23,42,.08), inset 0 1px 0 rgba(255,255,255,.6); }
