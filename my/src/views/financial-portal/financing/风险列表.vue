      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr v-for="(row, idx) in rows" :key="row.id">
            ...
            <td>{{ row.createdAt }}</td>
            <td class="op">
              <button class="btn xs" @click="goDetail(row.id)">风险处理</button>
              <button class="btn xs" @click="goDisposalList(row.id)">处置列表</button>
            </td>
          </tr>
        </tbody>
      </table>
...
function goDetail(id:number){ router.push(`/financial/financing/risk/handle/${id}`); }
function goDisposalList(id:number){ router.push(`/financial/financing/risk/disposal-list/${id}`); }
